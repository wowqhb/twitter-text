...
public class Autolink {
	...
	public void linkToText(Entity entity, CharSequence text, Map<String, String> attributes, StringBuilder builder) {
		...
		// append <a> tag
		builder.append("<a");
		String _target = " target=\"_blank\"";
		boolean _hasTarget = false;
		for (Map.Entry<String, String> entry : attributes.entrySet()) {
			String _tmp = entry.getKey();
			if (_tmp.equalsIgnoreCase("href")) {
				builder.append(" ").append(escapeHTML(_tmp)).append("=\"").append(entry.getValue()).append("\"");
			} else {
				builder.append(" ").append(escapeHTML(_tmp)).append("=\"").append(escapeHTML(entry.getValue())).append("\"");
			}
			if (_tmp.equalsIgnoreCase("target")) {
				_hasTarget = true;
			}
		}
		if (!_hasTarget) {
